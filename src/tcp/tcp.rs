...
use crate::pva_message::message_handler::handle_tcp_pva_msgs;
...
impl TcpMsgQueue {
    fn clear(&mut self) {
        match self {
            Self::Ca(queue) => queue.clear(),
            Self::Pva(queue) => queue.clear(),
        }
    }

    fn is_pva(&self) -> bool {
        matches!(self, Self::Pva(_))
    }

    fn drain(&mut self, max: usize) -> Self {
        match self {
            Self::Ca(queue) => {
                let count = queue.len().min(max);
                Self::Ca(queue.drain(..count).collect())
            }
            Self::Pva(queue) => {
                let count = queue.len().min(max);
                Self::Pva(queue.drain(..count).collect())
            }
        }
    }

    fn to_buf(self) -> Result<Vec<u8>, String> {
        let mut buf = Vec::new();
...
impl TCP {
    ...
    pub async fn disconnect(self: &Self, abort_read_task: bool, stop_check_alive_task: bool) {
        ...
        // Clear the send queue and remember which protocol owns this connection.
        let is_pva = {
            let mut queue = self.msg_que.write().unwrap();
            let is_pva = queue.is_pva();
            queue.clear();
            is_pva
        };
        ...
        {
            if is_pva {
                get_context().pva_tcps().remove_tcp(self.addr());
            } else {
                get_context().ca_tcps().remove_tcp(self.addr());
            }
        }
    ...
    async fn reconnect_channels(self: &Self) {
        ...
        if self.msg_que.read().unwrap().is_pva() {
            for cid in cids {
                if let Some(channel) = get_context().pva_channels().channel_by_cid(cid) {
            ...
            for cid in cids {
                if let Some(channel) = get_context().ca_channels().channel_by_cid(cid) {
                    // A lightweight await will not block for long, so sequential reconnects are
                    // acceptable here.
                    channel.reconnect().await;
                }
            }
    ...
    pub async fn start_to_listen_ca(self: Arc<Self>) {
    ...
    pub async fn start_to_listen_pva(self: Arc<Self>) {
        let mut read_task = self.read_task.lock().await;
        if read_task.is_some() {
            return;
        }
            ...
            loop {
                ...
                match num_bytes {
                    ...
                    Ok(mut size) => {
                        ...
                        let msgs =
                            PvaMsg::from_buf(&mut buf, Some(tcp.addr().clone()), vec![], true);
                        let src = *tcp.addr();
                        if !handle_tcp_pva_msgs(&src, msgs) {
                            buf.clear();
                        }
                    }
                    Err(err) => {
    ...
    async fn send_queue_msg(self: &Self) {
        let msgs = {
            let mut queue = self.msg_que.write().unwrap();
            queue.drain(100000)
        };

        let buf = match msgs.to_buf() {
            Ok(buf) if buf.is_empty() => return,
            Ok(buf) => buf,
            Err(err) => {
                error!("Failed to encode queued TCP message: {err}");
                return;
            }
        };

        if let Err(err) = self.send_buf(&buf).await {
            error!("Failed to send queued TCP messages: {err}");
        }
    }

    /**
     * Push Channel Access messages to this TCP connection's send queue.
     */
    pub fn send_ca_msgs(&self, msgs: Vec<CaMsg>) {
        if !self.is_connected() {
            return; // Err("TCP not connected".to_string());
        }

        match &mut *self.msg_que.write().unwrap() {
            TcpMsgQueue::Ca(queue) => queue.extend(msgs),
            TcpMsgQueue::Pva(_) => error!("Cannot queue CA messages on a PVA TCP connection"),
        }
    }

    /**
     * Push pvAccess messages to this TCP connection's send queue.
     */
    pub fn send_pva_msgs(&self, msgs: Vec<PvaMsg>) {
        if !self.is_connected() {
            return;
        }

        match &mut *self.msg_que.write().unwrap() {
            TcpMsgQueue::Pva(queue) => queue.extend(msgs),
            TcpMsgQueue::Ca(_) => error!("Cannot queue PVA messages on a CA TCP connection"),
        }
    }
    ...
    pub fn pva_endian(self: &Self) -> RwLockReadGuard<'_, Option<MsgEndian>> {
        self.pva_endian.read().unwrap()
    }

    pub fn set_pva_endian(self: &Self, endian: MsgEndian) {
        *self.pva_endian.write().unwrap() = Some(endian)
    }
...
impl TCPs {
     ...
     */
    pub async fn create_ca_tcp(self: &Self, addr: SocketAddr) -> Result<Arc<TCP>, String> {
            ...
            && tcp.is_connected()
        {
            ...
            .await
            {
                Ok(Ok(tcp)) => {
                    ...
                    tcp_listener.start_to_listen_ca().await;
                    ...
                    tcp.send_ca_msgs(vec![version_msg, client_name_msg, host_name_msg]);
    ...
    pub async fn create_pva_tcp(self: &Self, addr: SocketAddr) -> Result<Arc<TCP>, String> {
        if let Some(tcp) = self.tcp(&addr)
            && tcp.is_connected()
        {
            // this tcp already exists
            ...
            let (tcp, should_connect): (Arc<TCP>, bool) = {
                ...
                if let Some(tcp) = connecting.iter().find(|tcp| *tcp.addr() == addr).cloned() {
                    ...
                    let tcp: Arc<TCP> = Arc::new(TCP {
                        ...
                        alive: AtomicBool::new(true),
                        paused: AtomicBool::new(false),
                        msg_que: RwLock::new(TcpMsgQueue::Pva(VecDeque::new())),
                        pva_types_in: RwLock::new(PvaTypeRegistry::new()),
                        pva_types_out: RwLock::new(PvaTypeRegistry::new()),
                        pva_endian: RwLock::new(None),
                    });
            ...
            .await
            {
                Ok(Ok(tcp)) => {
                    ...
                    let tcp_listener = Arc::clone(&tcp);
                    tcp_listener.start_to_listen_pva().await;

                    let tcp_writer = Arc::clone(&tcp);
                    tcp_writer.start_to_write();

                    // client wait for CTRL_SET_ENDIANESS control message from server

                    // todo: do this after CMD_CONNECTION_VALIDATED
                    // let tcp_check_alive = Arc::clone(&tcp);
                    // tcp_check_alive.start_check_alive().await;

                    // client does not need to do anything after TCP connected
                    // send handshake
                    // let dests = vec![addr];
                    // let version_msg = CaMsg::build_version(&dests);
                    // let client_name_msg = CaMsg::build_client_name(&dests);
